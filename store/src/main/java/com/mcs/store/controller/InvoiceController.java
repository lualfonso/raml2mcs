...
public class InvoiceController {
    ...
    public ResponseEntity getInvoiceDetail(@PathVariable Long invoice_id ,@PathVariable Long detail_id )  {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Responded", "Invoice");
        
        InvoiceDetailGetDataModel wrapper = invoiceService.getInvoiceDetail(invoice_id,detail_id);        
        return ResponseEntity.ok().headers(headers).body(wrapper);
    }                            

    @PutMapping("/invoices/{invoice_id}/details/{detail_id}")
    public ResponseEntity putInvoiceDetail(@PathVariable Long invoice_id, @PathVariable Long detail_id,   @RequestBody InvoiceDetailsPutDataModel body ) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Responded", "Invoice"); 
    ...
    public ResponseEntity deleteInvoiceDetail(@PathVariable Long invoice_id ,@PathVariable Long detail_id ) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Responded", "Invoice");
        invoiceService.deleteInvoiceDetail(invoice_id, detail_id);
        return ResponseEntity.ok().headers(headers).build();
